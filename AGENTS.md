...
Offline, local-only **context migration** across enabled source adapters × enabled destination hosts (currently **17×18=306** cells, **derived from registries** — not a fixed product constant).
...
- **Installed-runner smoke** (registry-derived cell count; currently **17×18=306**): `smoke_installed_matrix.py` — packaging + installed `run_reader` only.
