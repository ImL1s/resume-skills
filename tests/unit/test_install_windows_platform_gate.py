"""Windows product mutation platform gate tests (Phase 7: real Windows enabled)."""
...
class WindowsPlatformGateTests(unittest.TestCase):
    def test_require_mutating_install_platform_fails_on_spoofed_nt(self) -> None:
        """Mocked nt on a non-Windows host still fails closed."""
    ...
    def test_require_mutating_install_platform_ok_on_real_windows(self) -> None:
        """Phase 7: real Windows (os.name == nt + sys.platform == win32) is allowed."""
        with mock.patch("portable_resume.install.transaction.os.name", "nt"), \
             mock.patch("portable_resume.install.transaction.sys") as mock_sys:
    ...
    def test_execute_install_mutation_fails_closed_on_spoofed_nt(self) -> None:
        """Mocked nt on non-Windows still fails closed."""
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_execute_install_dry_run_still_allowed_under_nt_mock(self) -> None:
        """Dry-run is always allowed regardless of platform."""
        with tempfile.TemporaryDirectory() as temporary:
            home = Path(temporary) / "home"
            project = Path(temporary) / "project"
            home.mkdir()
    ...
    def test_uninstall_mutation_fails_closed_on_spoofed_nt(self) -> None:
        """Mocked nt on non-Windows still fails closed."""
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_recover_with_journal_fails_closed_on_spoofed_nt(self) -> None:
        """Mocked nt on non-Windows still fails closed."""
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "skills"
    ...
    def test_recover_without_journal_noop_on_spoofed_nt(self) -> None:
        """No journal + mocked nt: noop regardless of platform gate."""
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "skills"
            root.mkdir()
            with mock.patch("portable_resume.install.transaction.os.name", "nt"), \
