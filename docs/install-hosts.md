...
This repository ships eleven `resume-<source>` Skills to eleven destination hosts (derived from registries). The reader remains offline and stdlib-only; plugin/marketplace packages contain the same inert Skills and no network integration.
...
The package builder creates eleven `*-<host>-skills.zip` archives (one per
enabled destination, including Pi, OpenClaw, and goose), seven supported
...
install-resume-skills quick-install all         # all eleven user-global profiles
...
For direct archives, extract the archive contents into the selected Skill root. Each archive contains `resume-antigravity`, `resume-claude`, `resume-codex`, `resume-cursor`, `resume-grok`, `resume-kimi`, `resume-opencode`, `resume-goose`, `resume-openclaw`, `resume-pi`, and `resume-qwen`.
...
Current `main` claims registry-derived **11×11=121** packaging and installed-runner
cells (including Pi, OpenClaw, and goose filesystem destinations). Published `0.3.4`
