...
O smoke local de hosts passou em 8/8 invocações de CLI; os pacotes exatos `0.3.2` passaram em 7/7 instalações nativas de plugin／extension compatíveis. A interação com seletores visuais e a publicação em marketplaces públicos continuam como verificações separadas ainda não executadas.
