...
| Host-neutral Skill payloads (#25) | [Issue #25](https://github.com/ImL1s/resume-skills/issues/25) | **Done on branch** `program/issue-25-host-neutral-skill-payload`: portable SKILL body; one package identity across hosts; Codex+Antigravity shared `.agents/skills` lifecycle. Residual: host-native activation smoke for shared payload still not-run. |
