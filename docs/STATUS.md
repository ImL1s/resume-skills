...
Current `main` advances to `0.4.3.dev0` after publishing immutable `v0.4.2`.
...
| OpenCode oversized live WAL (Issue #263) | **Phase 1 merged:** [PR #269](https://github.com/ImL1s/resume-skills/pull/269) → `fa897ca` adds honest `E_SQLITE_LIVE_WAL`, bounded accounting, and qualified independent fallback. **Phase 2 review/merge pending:** [PR #268](https://github.com/ImL1s/resume-skills/pull/268) uses a Darwin/APFS descriptor clone, clone-data-ID binding, pre-materialization private-main unlink, committed-WAL materialization, verified `/dev/fd` private open, identity-bound cleanup, first-entry bounded scratch rejection, and an exact-head proof JSON/checksum pair. Linux, Windows, non-APFS, cross-volume, missing-symbol, and capability-failure paths remain fail-closed; #263 is not closed until the revised exact-head CI artifact and fresh AI review pass. |
