...
| Python test suite | **783 pass in current main baseline CI (PR #216)** on Ubuntu/Python 3.12 ([run 30757382272](https://github.com/ImL1s/resume-skills/actions/runs/30757382272)); the same merge-head workflow passed Ubuntu/macOS Python 3.11–3.14, native Windows platform gates, and distribution smoke. The prior **944 local** snapshot at `18fe4ca` is historical, not a current-main count. v0.3.4 tag was 375. |
...
| Public marketplace catalog | **synced** [`portable-resume-marketplace@7833e4a`](https://github.com/ImL1s/portable-resume-marketplace/commit/7833e4a3628213f78f30e9873d43a95fa6) / [marketplace `v0.3.4`](https://github.com/ImL1s/portable-resume-marketplace/releases/tag/v0.3.4); fresh host reinstall **not-run** |
...
**Wave 2+ product (this land):** list `--limit`/`--since`/`--until`/`--cursor` (#157); `--workspace` + `project explain` (#154); `search` (#156); `pick` + `--privacy`/`--redaction-report` (#124); `config` layers/presets (#152); install `--sources` (#151); smoke `--mode source|destination|full` (#122); universal direct-skills zip by payload profile (#121); matrix docs gate (#119 partial→strengthened); native evidence policy doc (#123); Windows Policy B gate only (#29 done) — full Windows mutating install productization **#125 still OPEN**.  
**Cross-platform track — closed read-only/CI slices #205–#208; open residuals #125 and #209:** #205 `platform_fs` backend, #206 path/SQLite backend paths, #207 Windows **17-source read-only** list+show fixtures, and #208 CI on `ubuntu-latest` / `macos-latest` / `windows-latest` are **closed with evidence** for their stated read-only/CI surfaces. Windows read-only evidence: [Actions run 30753320460](https://github.com/ImL1s/resume-skills/actions/runs/30753320460) @ `a2412ed`; Phase‑1 native `LockFileEx` and full cross-platform suite evidence: [Actions run 30757382272](https://github.com/ImL1s/resume-skills/actions/runs/30757382272) @ PR [#216](https://github.com/ImL1s/resume-skills/pull/216). Production `stable_read_bytes` / `snapshot_sqlite_family` / `write_output_bytes` / request-file (Windows path) dispatch through the backend with honest capabilities. CI `test-windows` on `windows-latest` / Python 3.12 is real `nt` evidence (not Docker): compile + platform unit tests; `self-check` + `matrix`; `sources` / `doctor` / `discover`; hard gate `self_verify --only windows_source_fixtures` (all **17** enabled sources list+show on fixtures, or content-free diagnostic). Mutating `smoke_installed_matrix` is **skipped** on `windows-latest` **by design** — product install fails closed, not a green mutation claim. **#125 Phase 1 (landed, foundation only):** Win32 exclusive-lock primitive (`CreateFileW` + `LockFileEx` under `platform_fs`; `exclusive_locking`/`handle_locking` advertised). Product install/uninstall/recover remain Policy B fail-closed (`E_INSTALL_UNSUPPORTED_PLATFORM`). See [`docs/research/2026-08-03-windows-mutating-install-phase1-decision.md`](research/2026-08-03-windows-mutating-install-phase1-decision.md). **#125 remains OPEN / not product-complete:** do **not** claim #125 done until RootLock wiring + reparse-safe relative mutation (including lock-path parent-chain defenses) + adversarial `windows-latest` evidence land (Phase 2). **#209 umbrella remains OPEN:** WSL2, musl-only, FreeBSD/BSD independent runners remain **not-run**. Live resume / host UI / marketplace / Kilo source: **not claimed**.
...
| Windows native (`windows-latest` / real `nt`) | **verified** (read-only + Phase‑1 lock primitive) | **fail-closed** (#125 OPEN) | Read-only evidence [30753320460](https://github.com/ImL1s/resume-skills/actions/runs/30753320460); Phase‑1 lock evidence [30757382272](https://github.com/ImL1s/resume-skills/actions/runs/30757382272) |
| WSL2 | **not-run** | **not-run** | No dedicated runner; no fake green |
| musl-only | **not-run** | **not-run** | No dedicated runner |
| FreeBSD / BSD | **not-run** | **not-run** | No first-party runner |
...
| Windows install lock gate (#29) / #125 residual | [Issue #29](https://github.com/ImL1s/resume-skills/issues/29) · [Issue #125](https://github.com/ImL1s/resume-skills/issues/125) | **#29 Policy B gate done:** mutating install/uninstall/recover fail closed with `E_INSTALL_UNSUPPORTED_PLATFORM` on `os.name == "nt"` before support/lock creation; no silent unlocked mutation. **#125 Phase 1 (landed):** Win32 exclusive-lock primitive (`LockFileEx` under `platform_fs`) — product install still fail-closed. **#125 productization still OPEN** (RootLock wire + reparse-safe mutation not product-complete; not a Windows install-complete claim). Decision: [`research/2026-08-03-windows-mutating-install-phase1-decision.md`](research/2026-08-03-windows-mutating-install-phase1-decision.md). |
...
|---|---|---|
| Current checkout gates | pass | **783 tests in current main baseline CI (PR #216)** ([run 30757382272](https://github.com/ImL1s/resume-skills/actions/runs/30757382272), Ubuntu/Python 3.12) + **306/306** registry-derived installed-runner; native Windows passed the Phase‑1 lock primitive and read-only/platform gates while mutating installed-runner remained intentionally skipped. The prior **944 local** snapshot at `18fe4ca` is historical. |
