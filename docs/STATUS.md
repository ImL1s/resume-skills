...
| Gate | Status |
|---|---|
| Source adapters | 11: Claude, Codex, Cursor, OpenCode, Antigravity, Grok, Qwen, Kimi, Pi, OpenClaw, **goose** |
| Destination profiles | 11 including **Pi** + **OpenClaw** + **goose** (filesystem install supported) |
...
| goose source + destination (#39) | [Issue #39](https://github.com/ImL1s/resume-skills/issues/39) | **Done (filesystem/product path):** source `goose-sessions-sqlite-v15` (`sessions/sessions.db`); destination `.goose/skills` + `~/.config/goose/skills`; matrix 121 includes goose×all. Residual: legacy JSONL out of scope; native goose UI **not-run**. |
