...
| OpenCode oversized live WAL (Issue #263) | **Phase 1 merged:** [PR #269](https://github.com/ImL1s/resume-skills/pull/269) → `fa897ca` adds honest `E_SQLITE_LIVE_WAL`, bounded accounting, and qualified independent fallback. **Phase 2 review/merge pending:** Darwin/APFS descriptor-bound COW implementation and real exact-head proof job are in [PR #268](https://github.com/ImL1s/resume-skills/pull/268). Linux, Windows, non-APFS, cross-volume, missing-symbol, and capability-failure paths remain fail-closed; #263 is not closed until Phase 2 exact-head CI, proof artifact, and AI review pass. |
