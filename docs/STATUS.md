...
| Codex streaming show / reducer | **PR open** — [Issue #8](https://github.com/ImL1s/resume-skills/issues/8) + [plan 027](../plans/027-codex-streaming-show.md): plain show streams + reduces; zstd still whole decompress; true collect-free `stable_scan_lines` yield residual |
...
| Discovery false unsupported / stale SQLite | [Issue #7](https://github.com/ImL1s/resume-skills/issues/7) + plan 026 | **Done (P1a / PR #93):** probe uses DB signature without sessions walk; soft-capped sample for plain/zstd; byte-bounded head via `stable_read_windows` (`require_size_within_max=False`); FS soft merge/rank with `W_TRUNCATED` / fail-closed empty truncation. Show streaming remains #8. Residual P2: head parse line-count cap. |
| Peak memory on large show | [Issue #8](https://github.com/ImL1s/resume-skills/issues/8) + plan 027 | **PR open:** plain show uses `stable_scan_lines` + attempt-local reduce (no whole-file bytes / outer-record list); ~20 MiB synthetic regression. Residual: zstd still full decompress; `stable_scan_lines` still collect-then-yield per attempt. |
...
| Shared `stable_scan_lines` (#10) | [Issue #10](https://github.com/ImL1s/resume-skills/issues/10) + PR #49 | Foundation on main; **adopted by Pi, Kimi, Cursor CLI, Qwen, Grok, Antigravity, Codex plain show** for transcript JSONL. Claude private graph index retained. True collect-free yield remains residual. |
