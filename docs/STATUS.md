...
| Gate | Status |
|---|---|
| Source adapters | 16: Claude, Codex, Cursor, OpenCode, Antigravity, Grok, Qwen, Kimi, Pi, OpenClaw, goose, Crush, Cline, OpenHands, Hermes, **Gemini CLI** (Kilo source research-only) |
| Destination profiles | 18 including prior hosts + **GitHub Copilot CLI** (destination-only) + **Gemini CLI** + **Kilo CLI** (destination-only Track A) (filesystem install supported) |
| Packaging matrix | **288/288** on current main tip (**16×18**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
| Installed runner matrix | **288/288** on current main tip (**16×18**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
...
| Next-wave agent roadmap (Pi, OpenClaw, goose, …) | [Issue #48](https://github.com/ImL1s/resume-skills/issues/48) + [Issue #38](https://github.com/ImL1s/resume-skills/issues/38) | Phase 0 on `7b5192c`. **Pi / OpenClaw / goose / Crush / Cline / OpenHands / Hermes / Gemini / Copilot dest / Kilo dest** filesystem product paths done (matrix **16×18=288**). Native UI/picker activation not-run for these hosts. Remaining: Copilot source (#44 Track B), Kilo source (#46 Track B), … |
| Kilo destination Track A (#46) | [Issue #46](https://github.com/ImL1s/resume-skills/issues/46) | **Done (destination filesystem):** `.kilocode/skills` + `~/.config/kilo/skills` (`KILO_CONFIG_DIR`). Source stays research — no OpenCode adapter alias. Residual: core/effect SQLite schema pin + source adapter; native marketplace/UI **not-run**. |
