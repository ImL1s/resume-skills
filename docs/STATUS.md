...
| Destination root env homes (#24) | [Issue #24](https://github.com/ImL1s/resume-skills/issues/24) | **In progress:** Kimi `$KIMI_CODE_HOME/skills` + provenance in hosts report; isolation `--home` ignores host env. Residual: broader host env audit if docs add more overrides. |
