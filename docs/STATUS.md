...
| Installer control-plane pin/atomic (#21) | [Issue #21](https://github.com/ImL1s/resume-skills/issues/21) | **Done on POSIX** in branch work: support-dir pin, no-follow lock/journal/manifest, lock truncate, unique-tmp atomic replace. Residual: Windows exclusive lock productization (#29); optional previous-manifest generation journal enrichment. |
| Descriptor-relative install (#31) | [Issue #31](https://github.com/ImL1s/resume-skills/issues/31) + PR #49 | **Done on POSIX** for commit + rollback + orphan delete + uninstall + verify (dirfd/`O_NOFOLLOW`). Windows mutating ops fail closed without dirfd (#29). Classify/backup staging still uses no-follow file opens for sources; ambient `realpath` remains only for containment pre-checks, not for final mutate. |
