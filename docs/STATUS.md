...
| Capability registries + dynamic matrix | [Issue #36](https://github.com/ImL1s/resume-skills/issues/36) | Decouple source/destination/package surfaces; matrix cell count is `enabled_sources × enabled_destinations` (currently 8×8=64), not a hard-coded constant. In flight on PR #49 (Phase 0 / Milestone N1). |
| Shared `stable_scan_lines` (#10) | [Issue #10](https://github.com/ImL1s/resume-skills/issues/10) + PR #49 | Scanner foundation landed with integrity/budget tests; **not yet adopted by production adapters** (still private loops). True streaming yield remains open with #8. |
