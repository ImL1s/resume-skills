...
| Gate | Status |
|---|---|
| Source adapters | 15: Claude, Codex, Cursor, OpenCode, Antigravity, Grok, Qwen, Kimi, Pi, OpenClaw, goose, Crush, Cline, OpenHands, Hermes |
| Destination profiles | 16 including **Pi** + **OpenClaw** + **goose** + **Crush** + **Cline** + **OpenHands** + **Hermes** + **GitHub Copilot CLI** (destination-only) (filesystem install supported) |
| Packaging matrix | **240/240** on current main tip (**15×16**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
| Installed runner matrix | **240/240** on current main tip (**15×16**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
...
| Crush source + destination (#40) | [Issue #40](https://github.com/ImL1s/resume-skills/issues/40) | **Done (filesystem/product path):** source `crush-sqlite-v1` (per-project `.crush/crush.db`, goose_db_version 7); destination `.crush/skills` + `~/.config/crush/skills`; matrix **15×16=240**. Residual: native Crush UI **not-run**; no recursive multi-project home scan. |
