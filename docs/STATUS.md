...
**Cross-platform track (#205–#209 + #125):** `platform_fs` backend ships on main (PR #210–#212 + follow-on call-site migration). Production `stable_read_bytes` / `snapshot_sqlite_family` / `write_output_bytes` / request-file (Windows path) dispatch through the backend with honest capabilities. CI adds a dedicated `windows-latest` / Python 3.12 job (`test-windows`) for real `nt` read-only evidence — **not** Docker-as-Windows. Green tip evidence: [Actions run 30752320585](https://github.com/ImL1s/resume-skills/actions/runs/30752320585) @ `0b736ba` (self-check, matrix, multi-source fixture list/show, platform unit tests). Windows **mutating** install/uninstall/recover remains **fail-closed** (`E_INSTALL_UNSUPPORTED_PLATFORM`); #125 is **not** product-complete until handle locking + reparse-safe mutation land. WSL / musl / BSD independent runners: **not-run** (no GitHub runner faked green). Live resume / host UI / marketplace / Kilo source: **not claimed**.  
