...
| Multi-root install lock/checkpoint (#23) | [Issue #23](https://github.com/ImL1s/resume-skills/issues/23) | **Done on branch** `program/issue-23-multi-root-txn`: lock all unique physical roots in order before checkpoint/mutate; replan under locks; drift-safe compensation. Residual: no durable cross-root coordinator (same-process only). |
