...
| Gate | Status |
|---|---|
| Source adapters | 10: Claude, Codex, Cursor, OpenCode, Antigravity, Grok, Qwen, Kimi, Pi, **OpenClaw** |
| Destination profiles | 10 including **Pi** + **OpenClaw** (filesystem install supported) |
| Packaging matrix | **100/100** on current main tip (**10×10**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
| Installed runner matrix | **100/100** on current main tip (**10×10**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
...
| Capability registries + dynamic matrix | [Issue #36](https://github.com/ImL1s/resume-skills/issues/36) | **Done (product axes):** independent source/destination/package registries; rectangular matrix derived from enabled sets (currently **10×10=100** on main; published `0.3.4` was **9×9=81**); package surfaces drive native zip builds; schema source enum matches enabled sources (incl. `pi`, `openclaw`). Residual: auto-generated docs tables / planned-profile release gates still manual. |
...
| Next-wave agent roadmap (Pi, OpenClaw, goose, …) | [Issue #48](https://github.com/ImL1s/resume-skills/issues/48) + [Issue #38](https://github.com/ImL1s/resume-skills/issues/38) | Phase 0 on `7b5192c`. **Pi PR B/C done** (source + filesystem dest). **OpenClaw source+filesystem dest done** (`openclaw-agent-sqlite-v1`, matrix 10×10=100; native picker not-run). **goose PR A: fixtures-only** (`goose-sessions-sqlite-v15`; adapter #39 not landed). |
| OpenClaw source + destination (#37) | [Issue #37](https://github.com/ImL1s/resume-skills/issues/37) | **Done (filesystem/product path):** source `openclaw-agent-sqlite-v1`; destination workspace `skills/` + `~/.openclaw/skills`; matrix 100 includes OpenClaw×all. Residual: native `openclaw skills install` / host UI activation **not-run**. |
