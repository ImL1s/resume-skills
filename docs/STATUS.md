...
| Destination profiles | 15 including **Pi** + **OpenClaw** + **goose** + **Crush** + **Cline** + **OpenHands** + **Hermes** (filesystem install supported) |
| Packaging matrix | **225/225** on current main tip (**15×15**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
| Installed runner matrix | **225/225** on current main tip (**15×15**, derived from registries; published `0.3.4` remains historical **9×9=81**) |
...
| Crush source + destination (#40) | [Issue #40](https://github.com/ImL1s/resume-skills/issues/40) | **Done (filesystem/product path):** source `crush-sqlite-v1` (per-project `.crush/crush.db`, goose_db_version 7); destination `.crush/skills` + `~/.config/crush/skills`; matrix **15×15=225**. Residual: native Crush UI **not-run**; no recursive multi-project home scan. |
