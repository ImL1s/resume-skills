...
| Python test suite | **782 pass in current PR #215 CI** on Ubuntu/Python 3.12 ([run 30755777025](https://github.com/ImL1s/resume-skills/actions/runs/30755777025)); the same current-head workflow passed Ubuntu/macOS Python 3.11–3.14 and native Windows read-only gates. The prior **944 local** snapshot at `18fe4ca` is historical, not a current-main count. v0.3.4 tag was 375. |
...
|---|---|---|
| Current checkout gates | pass | **782 tests in current PR #215 CI** ([run 30755777025](https://github.com/ImL1s/resume-skills/actions/runs/30755777025), Ubuntu/Python 3.12) + **306/306** registry-derived installed-runner; the native Windows job passed read-only/platform gates while mutating installed-runner remained intentionally skipped. The prior 944 local snapshot at `18fe4ca` is historical. |
