...
| Codex probe head-only + list FS fallback | **in progress on PR #93** — [Issue #7](https://github.com/ImL1s/resume-skills/issues/7) + [plan 026](../plans/026-codex-probe-list-discovery.md); head window + soft probe sample + sparse/stale FS fallback pending exact-HEAD Codex re-review |
...
| Discovery false unsupported / stale SQLite | [Issue #7](https://github.com/ImL1s/resume-skills/issues/7) + plan 026 | **In progress (P1a / PR #93):** probe uses DB signature without sessions walk; soft-capped sample for plain/zstd evidence; byte-bounded head via `stable_read_windows`; FS head fallback on missing schema, unresolved/stale paths, or under-filled recognized DB. Show streaming remains #8. |
