...
| OpenCode oversized live WAL (Issue #263) | **Phase 1 merged:** [PR #269](https://github.com/ImL1s/resume-skills/pull/269) → `fa897ca` adds honest `E_SQLITE_LIVE_WAL`, bounded accounting, and qualified independent fallback. **Phase 2 review/merge pending:** [PR #268](https://github.com/ImL1s/resume-skills/pull/268) now uses a Darwin/APFS descriptor clone, committed-WAL materialization, exact `/.vol` private open, and identity-bound cleanup after addressing the first implementation review's P1/P2. Linux, Windows, non-APFS, cross-volume, missing-symbol, and capability-failure paths remain fail-closed; #263 is not closed until revised exact-head CI, proof artifact, and AI review pass. |
