...
| Next-wave agent roadmap (Pi, OpenClaw, goose, …) | [Issue #48](https://github.com/ImL1s/resume-skills/issues/48) + [Issue #38](https://github.com/ImL1s/resume-skills/issues/38) | Phase 0 on `7b5192c`. **Pi / OpenClaw / goose** filesystem product paths done (matrix **13×13=169**). Native UI/picker activation not-run for these hosts. Remaining wave agents: Crush/Cline/OpenHands/… |
...
| Crush source + destination (#40) | [Issue #40](https://github.com/ImL1s/resume-skills/issues/40) | **Done (filesystem/product path):** source `crush-sqlite-v1` (per-project `.crush/crush.db`, goose_db_version 7); destination `.crush/skills` + `~/.config/crush/skills`; matrix **13×13=169**. Residual: native Crush UI **not-run**; no recursive multi-project home scan. |
