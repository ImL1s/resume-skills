...
- `src/portable_resume/install/render.py` (lines ~96–100 at `a4dc4d6`, the
...
```
  ...
  The function reads package files from disk and renders templates on every
  call; its output is process-constant (pure function of the installed
  package tree, host-invariant in content).

  **#118 forward-compatibility (Codex round-4):** the in-flight
  `fix/issue-118-embedded-identity` branch adds an `identity` parameter plus
  `runtime_identity()` / `assert_identity_matches_package(...)` calls to this
  function. If your drift check shows `render.py` changed since `a4dc4d6`,
  adapt: key the cache on the canonical JSON serialization of the *resolved*
  identity (`json.dumps(identity, sort_keys=True, separators=(",", ":"))`)
  instead of a single process-constant entry, and keep the host validation
  AND any identity assertion outside the cached block so they still run per
  call. The rest of this plan is unchanged either way.
...
### Step 1: Add the cache in `render.py`

Against the current host-only API, one process-constant entry suffices:

```python
_PLAN_CACHE: dict[str, bytes] | None = None
...
The `dict(...)` shallow copy prevents caller mutation of the shared dict;
values are immutable `bytes`, so a shallow copy is sufficient. Keep the cache
module-private; add a `_reset_plan_cache()` helper for tests. The `host`
validation stays outside the cache so bad hosts still raise `KeyError`.

If the #118 identity surface has landed (see Current state), switch the
single entry to a `dict[str, dict[str, bytes]]` keyed on the canonical
identity serialization, and keep the identity assertion outside the cached
block so it still runs per call — only the file-read/render work is cached.
...
2. Cross-host equality survives the cache: `package_identity(materialize_plan("claude")) == package_identity(materialize_plan("qwen"))`
   and both calls succeed after the cache is warm.
3. `_reset_plan_cache()` in `setUp` of any test module that mutates the
   package tree expectation, to keep isolation. (If the #118 identity
   parameter has landed, add: two different synthetic identities produce
   different cache entries — build them the way existing identity tests do.)
...
- [ ] Host validation (and the identity assertion, if #118 landed) runs per
      call — outside the cached block
